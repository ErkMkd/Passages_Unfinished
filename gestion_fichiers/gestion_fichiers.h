...
class Fichiers
{
    ...
    {
        int32_t i=0;
        for (;i<taille_max;i++) if (chaine_source[i]==0) break;
        char* chaine=(char*)malloc(i+1);
        if (chaine==NULL)return NULL;
        for (;i>=0;i--) chaine[i]=chaine_source[i];
        return chaine;
    }
    ...
    static int32_t charge_donnees_binaires(char* tempon,uint32_t taille_tempon, char* nom_fichier)
    {
        ...
        {
            std::cout<<"ERREUR - LWO2 - Impossible d'ouvrir le fichier "<<nom_fichier<<std::endl;
            return false;
        }
